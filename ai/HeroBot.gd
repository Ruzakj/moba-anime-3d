...
func _act()->void:
	match state:
		...
		State.PUSH,State.MOVE_TO_LANE:
			...
			var p=lane_points[clamp(lane_index,0,lane_points.size()-1)]
			_move_to(p)
			if hero.global_position.distance_to(p)<2.2 and lane_index<lane_points.size()-1:lane_index+=1

func _move_to(p:Vector3)->void:
	var dir=p-hero.global_position;dir.y=0
	if dir.length()>0.2:
		hero.velocity=dir.normalized()*hero.move_speed
		hero.look_at(hero.global_position+hero.velocity,Vector3.UP)
		hero.move_and_slide()
